...
\input{figures/resnet50_cancer_expert_confusion_matrix_comparison_figure}
...
in Figure~\ref{fig:results-metric-comparison}. There we can see, that the
cancer expert system improves over all measured metrics but epithilum recall.
However, since we detect cancer by detecting both epithelium and stroma and the
lower recall for epithelium is due to the fact that some epithelium samples are
now misclassified as stroma, this is an acceptable trade-off.
\input{figures/results_metric_comparison_figure}
