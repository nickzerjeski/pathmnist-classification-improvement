...
reliably identify the cancer class, overall accuracy alone is not sufficient. A
model may achieve high accuracy while still missing a relevant number of
cancerous samples, especially if errors are unevenly distributed across classes.
For this reason, special attention is given to the classification performance
for colorectal adenocarcinoma epithelium. In particular, recall-oriented metrics
are important because false negatives, where cancerous tissue is classified as
non-cancerous, are especially problematic in a medical context. This makes the
dataset suitable for evaluating whether a classifier can not only perform well
globally, but also prioritize the reliable detection of cancerous tissue.
