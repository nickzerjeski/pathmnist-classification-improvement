...
the primary baseline throughout this work. The resulting baseline metrics are
shown in Table~\ref{tab:baseline-metrics}. \input{tables/baseline_metrics}

The baseline results indicate that the largest improvement potential lies in the
classification of stroma. While the overall accuracy and the cancer-vs.-rest
metrics are already high, stroma recall is only $0.544$ and stroma $F_2$ is only
$0.592$. This means that almost half of the stroma samples are not correctly
identified by the baseline model. In contrast, epithelium is detected much more
reliably, with a recall of $0.967$ and an $F_2$ score of $0.961$. Therefore,
improving the recognition of stroma is likely the most important direction for
increasing the cancer-related performance of the model we are building.
