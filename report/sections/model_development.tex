...
architecture is shown in Figure~\ref{fig:cancer-expert}.
Each expert uses the same small CNN architecture as the base model, but with a
binary output layer, in essence stroma vs. the competing class. Therefore, the
expert classifiers are only trained on samples from its own two ground-truth
classes.
\input{figures/cancer_expert}
